...
Now test from Prom&Grafana VM side:
Browse - http://192.168.126.131:9100/metrics
...
Choose Prometheus
Provide default Prometheus URL: http://localhost:9090
...
Set Up Alert Notifications via Teams:
...
Step 2: Add a Microsoft Teams Notification Channel in Grafana
...
STEP 3: Create an Alert Rule for core components like CPU, Memory, and Disk
...
2. Define query and alert condition 
   ...
   Condition: IS ABOVE
   Value: 70 (Set the Value as 70 if you want to trigger a notification once the component usage reaches above the defined threshold)
...
6. Configure notification message
   ...
   eg - High CPU Usage on {{ $labels.instance }}
   Description (optional)
   Description of what the alert rule does.
   eg - CPU usage has crossed 70% on {{ $labels.instance }} for more than 2 minutes. Please check running processes or workloads on the node.
7. Save Rule & Exit   
